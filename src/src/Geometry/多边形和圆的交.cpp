double sector_area (cp a, cp b, double r) {
	...
	double al = acos (c);
	return r * r * al / 2.0; }
double area(cp a,cp b, double r) { // point() : (0, 0)
	double dA = dot (a, a), dB = dot (b, b), dC = point_to_segment (point (), line (a, b)), ans = 0;
...
double solve(const vector<point> &p, cc c) {//p 逆时针
	double ret = 0;
