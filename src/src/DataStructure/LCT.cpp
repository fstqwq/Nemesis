struct Node {
	Node *fa, *top, *c[2], *mi; int val; bool rev, isrt;
	...
	void revIt() { rev ^= 1; swap(c[0], c[1]); }
	void relax() {
		if (rev) c[0]->revIt(), c[1]->revIt(), rev = 0; }
	void setrt(Node *f);
} Tnull, *null = &Tnull, mem[N + M], *ptr = mem;
...
void rot(Node *x) {
	...
	x->setc(fa, !d);
	fa->upd();
	if (fa->isrt) fa->isrt = 0, x->isrt = 1, x->top = fa->top; } 
void pushto(Node *x) {
	static Node *st[N]; int top = 0;
...
void splay(Node *x) {
	...
	x->upd(); }
void expose(Node *x) {
	for (Node *y = null; x != null; y = x, x = x->top)
		splay(x), x->c[1]->setrt(x); x->setc(y, 1); }
void mkrt(Node *x) { expose(x); splay(x); x->revIt(); }
void Link(Node *u, Node *v) { mkrt(u); u->top = v; }
void Cut(Node *u, Node *v) {
	mkrt(u); expose(v); splay(u);
	u->setc(null, 1); u->upd();
	v->setrt(null); }
Node *query(Node *u, Node *v) {
	mkrt(u); expose(v); splay(v); return v->mi;}
Node *getrt(Node *x) {
	expose(x); splay(x);
...
int main() {
	...
	for (int i = 1, cnt = 0, j = 1; i <= m; i++) {
		if (getrt(a[e[i].u]) != getrt(a[e[i].v])) { cnt++; newE(i); }
		else if (e[i].u != e[i].v){
			Node *mi = query(a[e[i].u], a[e[i].v]); delE(mi); newE(i); }
		while (!vis[j]) j++;
