		...
		\begin{spacing}{0.6}
			...
			\section{Geometry}
				...
				\subsection{整数半平面交}
				 ...
				 \subsection{线段在多边形内 (Airport Construction)}
...
%				\inputminted{cpp}{src/Geometry/最近点对.cpp}
			...
			\section{Tree \& Graph}
				\subsection{Hopcroft-Karp $O(\sqrt{V} E)$}
					\inputminted{cpp}{src/TreeandGraph/hk_skip2004.cpp}
			...
			\section{String}
				...
				\texttt{\input{|"python src/Miscellany/randomprimes.py 1e12 1e13 2e13 1e17 1e18"}}
			...
			\section{Math 数学}
				...
				\subsection{线性递推}
					...
					\subsubsection*{$O(k \log k \log n)$}
						需要抄前面的多项式取模。预处理 $O(k \log k \log n)$，固定 $n$ 和系数只改变初始值的话，询问一次 $O(k)$。
			...
			\section{Miscellany}
				...
				\begin{small}
					...
					\texttt{\input{|"python src/Miscellany/randomprimes.py 500 1e3 3e4 1e5 1e6 5e6 1e7 2e7 1e9 2e9"}}
