...
inline void updata(int x){
  if(p[x].l){
    for(int i=0;i<2;i++){
      ...
      Max(p[x].max[i],p[p[x].l].max[i]);}}
  if(p[x].r){
    for(int i = 0;i<2;i++){
      Min(p[x].min[i],p[p[x].r].min[i]);
      Max(p[x].max[i],p[p[x].r].max[i]);}}}
...
void insert(int now,int D){
  if(p[now].d[D]>=p[n].d[D]){
    ...
    else p[now].l=n;updata(now);
  }else{
    if(p[now].r)insert(p[now].r,D^1);
    else p[now].r=n;updata(now);}}
...
void ask1(int now){
  int pl=inf,pr=inf;
  Min(ans,abs(x-p[now].d[0])+abs(y-p[now].d[1]));
  ...
  if(pl<pr){
    ...
    if(pl<ans)ask(p[now].l);}}
void ask2(int now){
  if(x1<=p[now].min[0]&&x2>=p[now].max[0]&&
     y1<=p[now].min[1]&&y2>=p[now].max[1]){
    ans+=p[now].sum;return;}
  if(x1>p[now].max[0]||x2<p[now].min[0]||
     y1>p[now].max[1]||y2<p[now].min[1])return;
  if(x1<=p[now].d[0]&&x2>=p[now].d[0]&&
     y1<=p[now].d[1]&&y2>=p[now].d[1])
    ans+=p[now].val;
