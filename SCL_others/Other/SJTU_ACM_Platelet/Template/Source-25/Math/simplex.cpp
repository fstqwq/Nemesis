  ...
  for(double d;true;){
    ...
    for(int i = 0;i<n;i++)if(D[i][s]<-eps){
        if(r<0||
           ...
           -eps||d<eps&&ix[r+m]>ix[i+m]){r = i;}}
    if(r<0){
      return/* unbounded */std::vector<double>();}}
  if(D[n+1][m]<-eps){
    return/* no solution */std::vector<double>();}
  std::vector<double> x(m-1);
  for(int i = m;i<n+m;i++)
    if(ix[i]<m-1){x[ix[i]] = D[i-m][m];} return x;}
