...
struct LinearRec {
  int n,LOG;poly first,trans;vector <poly> bin;
  poly add(poly &a,poly &b){
    ...
    return res;}
  LinearRec(poly &first,poly &trans,int LOG): LOG(
    LOG),first(first),trans(trans){
    n=first.size();poly a(n+1,0);a[1]=1;
    bin.push_back(a);
    for(int i=1;i<LOG;i++)
      bin.push_back(add(bin[i-1],bin[i-1]));}
  int calc(long long k){
    poly a(n+1,0);a[0]=1;int ret=0;
    for(int i=0;i<LOG;i++)
      if((k>>i)&1)a=add(a,bin[i]);
    for(int i=0;i<n;i++)
      (ret+=1ll*a[i+1]*first[i]%mo)%=mo;
    return ret;}};
