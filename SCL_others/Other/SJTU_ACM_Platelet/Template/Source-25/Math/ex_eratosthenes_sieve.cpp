...
LL calc(LL w,int id,LL f){
  ...
  for(int i = id;i<P.size();++i){
    int p = P[i],e = 1; LL q = (LL)p*p;
    if(q>w) break;
    ret += calc(w/p,i+1,f*fpk(p,1));
    while(1){
      ++e;LL f2 = f*fpk(p,e);ret += f2;LL qq = q*p;
      if(qq<=w){ret += calc(w/q,i+1,f2);q = qq;
      }else break;}}
  return ret;}
void prepare(LL N){ // about O(n^0.67)
  n = N; m = (int)sqrt(n+.5L);
  A.resize(m+1); B.resize(m+1); P.clear();
  vector<int> isp; isp.resize(m+1,1);
  for(int i = 1;i<=m;++i){
  ...
  for(int p = 2;p<=m;++p){
    ...
    for(int i=1;i<=m/p;++i)B[i] = B[i]-(B[i*p]-d)*p;
    for(int i=m/p+1;i<=to;++i)
        B[i]=B[i]-(A[n/p/i]-d)*p;
    for(int i=m;i>=p2;--i)A[i] = A[i]-(A[i/p]-d)*p;}}
