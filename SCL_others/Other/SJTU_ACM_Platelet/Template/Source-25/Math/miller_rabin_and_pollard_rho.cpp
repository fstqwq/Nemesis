int64_t
multiply_mod(int64_t x,int64_t y,int64_t p){
  int64_t t =
    (x*y-(int64_t)((long double)x/p*y+1e-3)*p)%p;
  return t<0 ? t+p : t; }
int64_t add_mod(int64_t x,int64_t y,int64_t p){
  return (0ull+x+y)%p; }
int64_t
power_mod(int64_t x,int64_t exp,int64_t p){
  int64_t ans = 1;
  while(exp){
    if(exp&1) ans = multiply_mod(ans,x,p);
    x = multiply_mod(x,x,p); exp >>= 1;}
  return ans; }
bool
miller_rabin_check(int64_t prime,int64_t base){
  int64_t number = prime-1;
  for(;~number&1;number >>= 1) continue;
  int64_t result = power_mod(base,number,prime);
  for(;
    number!=prime-1&&result!=1&&result!=prime-1;
    number <<= 1)
...
bool miller_rabin(int64_t number){
  ...
  for(int i = 0;
      i<test_case_size&&test_cases[i]<number;i++)
...
int64_t
pollard_rho_test(int64_t number,int64_t seed){
...
void factorize(int64_t number,
  ...
  if(number>1){
    if(miller_rabin(number)){
      ...
      while(factor>=number)
        factor = pollard_rho_test(number,
                rand()%(number-1)+1);
