struct blossom {
  struct Edge {int x,y,next;}edge[M];
  int n,W,tot,h,l,son[N];
  int mat[N],pre[N],tp[N],q[N],vis[N],F[N];
  void Prepare(int n_){n=n_;W=tot=0;
    for(int i=1;i<=n;i++)son[i]=mat[i]=vis[i]=0;}
  ...
  void blo(int u,int v,int f){
    for(int w;find(u)^f;u=pre[v=w]){
      pre[u]=v,F[u]?0:F[u]=f;F[w = mat[u]]?0:F[w]=f;
      tp[w]^1?0:tp[q[++l]=w]=-1;}}
  int bfs(int x){
