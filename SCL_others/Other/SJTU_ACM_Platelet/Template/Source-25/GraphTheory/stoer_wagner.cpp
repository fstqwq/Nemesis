...
int get(int &s,int &t){
  ...
  for(i=1;i<=n;i++){k=mx=-1;
    for(j=1;j<=n;j++)
      if(!g[j]&&!v[j]&&d[j]>mx)k=j,mx=d[j];
    if(k==-1)return an;
    s=t;t=k;an=mx;v[k]=1;
    for(j=1;j<=n;j++)
      if(!g[j]&&!v[j])d[j]+=w[k][j];}return an;}
int mincut(int n,int w[N][N]){
  //n 为点数, w[i][j] 为 i 到 j 的流量
  //返回无向图所有点对最小割之和
  int ans=0,i,j,s,t,x,y,z;
  for(i=1;i<=n-1;i++){
    ans=min(ans,get(s,t));g[t]=1;if(!ans)break;
    for(j=1;j<=n;j++)
      if(!g[j])w[s][j]=(w[j][s]+=w[j][t]);}
  return ans;}
// 无向图最小割树
