...
struct Convex {
  int n; vector <Point> a,upper,lower;
  ...
  pair<long long,int> get_tangent(
          vector<Point> &convex,Point vec){
    int l = 0,r = (int)convex.size()-2;
    for(;l+1<r;){
      int mid = (l+r)/2;
      if(sign((convex[mid+1]-convex[mid]).det(vec))
         >0) r = mid;
      else l = mid; }
    return max(make_pair(vec.det(convex[r]),r),
               make_pair(vec.det(convex[0]),0)); }
  void update_tangent(const Point &p,int id,int &i0,
                 int &i1){
    if((a[i0]-p).det(a[id]-p)>0) i0 = id;
    if((a[i1]-p).det(a[id]-p)<0) i1 = id; }
  void binary_search(int l,int r,Point p,int &i0,
                     int &i1){
    if(l==r) return;
    ...
    for(;l+1<r;){
      int mid = (l+r)/2;
      int smid =
        sign((a[mid%n]-p).det(a[(mid+1)%n]-p));
  ...
  bool contain(Point p){
    ...
    }else if((lower[id-1]-p).det(lower[id]-p)<0)
      return false;
    id = lower_bound(upper.begin(),upper.end(),Point(
            p.x,INF),greater<Point>())-upper.begin();
    if(upper[id].x==p.x){
      if(upper[id].y<p.y) return false;
    }else if((upper[id-1]-p).det(upper[id]-p)<0)
      return false;
    return true; }
  // 求点 p 关于凸包的两个切点
  // 如果在凸包外则有序返回编号
  ...
  int get_tangent(Point vec){
    ...
    return ret.second; }
  // 求凸包和直线u,v的交点, 如果无严格相交返回false
  // 如果有则是和 (i,next(i)) 的交点, 两个点无序，
  // 交在点上不确定返回前后两条线段其中之一
  bool get_intersection(Point u,Point v,int &i0,
                        int &i1){
    int p0 = get_tangent(u-v), p1 = get_tangent(v-u);
    if(sign((v-u).det(a[p0]-u))*
       sign((v-u).det(a[p1]-u))<0){
      if(p0>p1) swap(p0,p1);
