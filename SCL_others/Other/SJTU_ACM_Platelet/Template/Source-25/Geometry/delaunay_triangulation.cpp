...
double dist_sqr(P const &a,P const &b);
bool in_circumcircle(P const &p1,P const &p2,
    P const &p3,P const &p4){//p4 in C(p1,p2,p3)
  ...
  return det>eps; }
double side(P const &a,P const &b,P const &p){
  return (b.x-a.x)*(p.y-a.y)-(b.y-a.y)*(p.x-a.x); }
typedef int SideRef;
struct Triangle;
typedef Triangle *TriangleRef;
struct Edge {
  TriangleRef tri; SideRef side;
  Edge(): tri(0),side(0){}
  Edge(TriangleRef tri,SideRef side):
      tri(tri),side(side){} };
struct Triangle {
  P p[3]; Edge edge[3]; TriangleRef children[3];
  Triangle(){}
  ...
  bool contains(P const &q) const{
    double a = side(p[0],p[1],q),
      b = side(p[1],p[2],q),c = side(p[2],p[0],q);
...
class Triangulation {
 public:
  Triangulation(){
    ...
    the_root = new(tot_triangles++) Triangle(
 ...
 private:
  ...
  static TriangleRef find(TriangleRef root,P p){
    for(;;){
  ...
  void add_point(TriangleRef root,P const &p){
    ...
    tab = new(tot_triangles++)
          Triangle(root->p[0],root->p[1],p);
    tbc = new(tot_triangles++)
          Triangle(root->p[1],root->p[2],p);
    tca = new(tot_triangles++)
          Triangle(root->p[2],root->p[0],p);
  ...
  void flip(TriangleRef tri,SideRef pi){
    ...
    if(!trj||!in_circumcircle(
        tri->p[0],tri->p[1],tri->p[2],trj->p[pj]))
      return;
    TriangleRef trk = new(tot_triangles++)
     Triangle(tri->p[(pi+1)%3],trj->p[pj],tri->p[pi]);
    TriangleRef trl = new(tot_triangles++)
     Triangle(trj->p[(pj+1)%3],tri->p[pi],trj->p[pj]);
