...
struct Complex {
  ...
  Complex operator*(const Complex &that) const{
    return (Complex){x*that.x-y*that.y,
                     x*that.y+y*that.x}; }
  Complex operator+(const Complex &that) const{
    return (Complex){x+that.x,y+that.y}; }
  ...
  Complex operator-(const Complex &that) const{
    return (Complex){x-that.x,y-that.y}; }
} buf_a[maxn],buf_b[maxn],buf_c[maxn],w[maxn],
...
void init(){
  int h = n>>1;
  for(int i = 0;i<h;++i)
    w[i+h] = (Complex){cos(2*pi*i/n),sin(2*pi*i/n)};
...
int main(){
  ...
  bit_reverse(a,buf_a); bit_reverse(b,buf_b);
  init(); dft(buf_a); dft(buf_b);
