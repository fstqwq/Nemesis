\section{莫队\ \small(ct)}
    \myminted{cpp}{DataStructure/mo_dui.cpp}
\section{长链剖分\ \small(ct)}
    \myminted{cpp}{DataStructure/long_chain.cpp}
\section{DSU\ \small(ct)}
    \myminted{cpp}{DataStructure/dsu_on_tree.cpp}
\section{带权并查集\ \small(ct)}
    \myminted{cpp}{DataStructure/union_find_weighted.cpp}
%\section{可并堆\ \small(ct)}
    %\myminted{cpp}{DataStructure/mergeable_heap.cpp}
\section{Splay\ \small(ct)}
    \myminted{cpp}{DataStructure/splay.cpp}
\section{线段树\ \small(ct)}
    %\subsection*{zkw线段树}
        %$ 0 $-based
        %\myminted{cpp}{DataStructure/zkw_segment_tree.cpp}
    %\subsection*{李超线段树}
        %\myminted{cpp}{DataStructure/li_chao_segment_tree.cpp}
    \subsection*{吉利线段树}
...
\section{二进制分组\ \small(ct)}
    ...
    \myminted{cpp}{DataStructure/binary_group.cpp}
\section{CDQ分治\ \small(ct)}
    \myminted{cpp}{DataStructure/cdq_divide.cpp}
\section{斜率优化\ \small(ct)}
    ...
    %\subsection*{单调队列}
    \myminted{cpp}{DataStructure/slope_optimize_queue.cpp}
    %\subsection*{线段树}
        %\myminted{cpp}{DataStructure/slope_optimize_segment.cpp}
\section{树分块\ \small(ct)}
    树分块套分块：给定一棵有点权的树，每次询问链上不同点权个数
    \myminted{cpp}{DataStructure/tree_block.cpp}
\section{KD\ tree\ \small(lhy)}
    \myminted{cpp}{DataStructure/kd_tree.cpp}
\section{DLX\ \small(Nightfall)}
    \myminted{cpp}{DataStructure/dlx.cpp}
