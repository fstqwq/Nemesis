// 链上push_back等差数列，链上查最小值
int size[maxn],dep[maxn],son[maxn],fa[maxn],
  top[maxn],dfn[maxn],pos[maxn],timer,rig[maxn];
ll dis[maxn]; bool vis[maxn];
void dfs1(int x);
void dfs2(int x){cmax(rig[top[x]],dfn[x]);}
inline int getlca(int a,int b); // 树链剖分
struct Seg {
  Seg *ls,*rs; ll min,k,b,vl,vr;
  // min 表示区间最小值
  // k，b 表示区间内 直线标记的斜率和截距
...
void build(int l,int r){
  Seg *o=scnt;o->k=0;o->b=inf;
  o->vl=dis[pos[l]];o->vr=dis[pos[r]];
  o->min=inf;if(l==r)return;int mid=l+r>>1;
  o->ls=++scnt;build(l,mid);
  o->rs=++scnt;build(mid+1,r);
  o->update(); }
int ql,qr,qk; ll qb;
void modify(Seg *o,int l,int r,int k,ll b){
  int mid=l+r>>1; if(ql<=l&&r<=qr){
    if(l==r){cmin(o->min,k*o->vl+b);return;}
    ll val=o->vl*k+b,var=o->vr*k+b,
      vbl=o->vl*o->k+o->b,vbr=o->vr*o->k+o->b;
    if(val<=vbl&&var<=vbr){
      o->k=k;o->b=b;o->update();return;}
    if(val>=vbl&&var>=vbr) return;
    ll dam=dis[pos[mid]],vam=dam*k+b,
       vbm=dam*o->k+o->b;
    if(val>=vbl&&vam<=vbm){
      modify(o->ls,l,mid,o->k,o->b);o->k=k;o->b=b;}
    else if(val<=vbl&&vam>=vbm)
      modify(o->ls,l,mid,k,b);
    else{if(vam<=vbm&&var>=vbr){
        modify(o->rs,mid+1,r,o->k,o->b);
        o->k=k;o->b=b;}
      else modify(o->rs,mid+1,r,k,b);}
    o->update();return;}
