...
int popcnt[256];
inline void set(uint *s,int pos){
  s[pos>>5] |= 1u<<(pos&31);
}
inline int popcount(uint x){
  return popcnt[x>>24&255]+popcnt[x>>16&255]+
         popcnt[x>>8&255]+popcnt[x&255];
}
void main(){
  int n,q;
  scanf("%d%d",&n,&q);
...
#define modify(x,_p)\
  ...
  for(int Q = 0;Q<q;++Q){
    ...
    int i = x>>5,j = y>>5,p,lim;
    for(p = 0,lim = l>>5;p+8<lim;
        p += 8,i += 8,j += 8){
