...
struct Seg {
  ...
  inline void add(const P &that){
    int top = v.size();
    P *v = this->v.data()-1;
    while(top>1&&slope(v[top-1],v[top])>
                 slope(v[top],that))
      --top;
    this->v.erase(this->v.begin()+top,
                  this->v.end());
...
inline ll Query(int o,int l,int r){
  if(ql<=l&&r<=qr&&d[tmp]-d[pos[r]]>len)
    return inf;
  ...
  int mid = l+r>>1;
  if(ql<=mid)
    temp = Query(o<<1,l,mid),cmin(ret,temp);
  if(mid<qr)
    temp = Query(o<<1|1,mid+1,r),cmin(ret,temp);
  return ret;
}
inline ll calc(){
  ll ret = inf;
