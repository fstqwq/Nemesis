...
int sum[maxn],lmx[maxn],rmx[maxn],mx[maxn],
  v[maxn],id[maxn],root;
...
inline void pushdown(int x){
  ...
  if(tag[x]){
    rev[x] = tag[x] = 0;
    if(ls)
      tag[ls] = 1,v[ls] = v[x],sum[ls] =
        size[ls]*v[x];
    if(rs)
      tag[rs] = 1,v[rs] = v[x],sum[rs] =
        size[rs]*v[x];
    if(v[x]>=0){
...
int main(){
  ...
  for(int i = 1;i<=m;i++){
    ...
    if(op=='M'&&op2=='X'){
      ...
      if(op=='M')
        val = FastIn(),Makesame(l,tot,val);
      if(op=='R') Reverse(l,tot);
      if(op=='G') Query(l,tot);
    }
  }
  return 0;
