...
void insert(const std::vector<int> &V,int rownum){
  ...
  for(int i = 0;i<int(V.size());++i){
    node *now = Node+(totNode++);
    now->row = rownum;
    now->col = now->up = col[V[i]];
    now->down = col[V[i]]->down;
    now->up->down = now,now->down->up = now;
    now->col->cnt++;
    N.push_back(now);
  }
  for(int i = 0;i<int(V.size());++i){
    N[i]->right = N[(i+1)%V.size()];
    N[i]->left = N[(i-1+V.size())%V.size()];
  }
}
void Remove(node *x){
...
bool search(int tot){
  if(head->right==head) return ansNode = tot,true;
  node *choose = NULL;
  for(node *i = head->right;i!=head;i = i->right){
    if(choose==NULL||choose->cnt>i->cnt)
      choose = i;
    if(choose->cnt<2) break;
  }
  Remove(choose);
  for(node *i = choose->down;i!=choose;
      i = i->down){
    for(node *j = i->right;j!=i;j = j->right)
      Remove(j->col);
    ans[tot] = i;
...
// prepare(C);for(i(rows))insert({col_id},C);
// search(0);
