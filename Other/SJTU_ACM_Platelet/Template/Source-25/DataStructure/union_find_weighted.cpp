struct edge { int a,b,w;
  inline bool operator<(const edge &that) const{
    return w>that.w; } } e[maxm];
int fa[maxn],f1[maxn],f2[maxn],f1cnt,f2cnt,
  val[maxn],size[maxn];
int main(){
  ...
  for(int i=1;i<=m;++i){
    ...
    if(x!=y){
      ...
      size[x]+=size[y];val[y]=e[i].w;fa[y]=x;}}
  int q; scanf("%d",&q);
  for(;q;--q){
    ...
    while(f1cnt&&f2cnt&&f1[f1cnt]==f2[f2cnt])
      --f1cnt,--f2cnt; int ret = 0x7fffffff;
