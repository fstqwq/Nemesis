...
#define nil mem
struct Node {
  Node *l,*r; int dist; int x,y; Val val,laz;
} mem[M] = {{nil,nil,-1}};
int sz = 0;
#define NEW(arg...) (new(mem + ++ sz)Node{nil,nil,0,arg})
void add(Node *x,Val o){
  if(x!=nil) x->val += o,x->laz += o;}
void down(Node *x){
  add(x->l,x->laz); add(x->r,x->laz); x->laz = 0; }
...
Val chuliu(int s,int n){ // O(ElogE)
  ...
  ++rid;
  for(int i = 1;i<=n;++i) if(i!=s&&S[i]==i)
