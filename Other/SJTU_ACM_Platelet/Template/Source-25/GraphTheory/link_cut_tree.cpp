...
struct Node {
  ...
  bool rev;
  // min_val in chain
  inline bool type(){ return fa->ch[1]==this; }
  inline bool check(){ return fa->ch[type()]==this; }
  inline void pushup(){
    pos = this;
    mn = val;
  ...
  inline Node *access(){
    Node *i = this,*j = null;
    for(;i!=null;i = (j = i)->fa){
      i->splay(); i->ch[1] = j; i->pushup(); }
    return j; }
  inline void make_root(){
    access(); splay(); rev ^= 1; }
  inline void link(Node *that){
    make_root(); fa = that; splay(0); }
  inline void cut(Node *that){
    make_root(); that->access(); that->splay(0);
    that->ch[0] = fa = null; that->pushup(); }
} mem[maxn];
