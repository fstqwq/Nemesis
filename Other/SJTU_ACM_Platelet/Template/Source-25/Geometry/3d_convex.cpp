...
double volume(int a,int b,int c,int d){
  return mix(info[b]-info[a],info[c]-info[a],
             info[d]-info[a]);}
struct Face {
  int a,b,c;
  Face(){}
...
P findCenter(){
  ...
  P center(.0,.0,.0); P first = info[face[0][0]];
  for(int i = 0;i<SIZE(face);++i){
    P p = (info[face[i][0]]+info[face[i][1]]+
           info[face[i][2]]+first)*.25;
    double weight = mix(info[face[i][0]]-first,
                        info[face[i][1]]-first,
                        info[face[i][2]]-first);
    totalWeight += weight;
...
double minDis(P p){
  ...
  for(int i = 0;i<SIZE(face);++i)
    res = min(res, calcDist(p,face[i][0],face[i][1],
                            face[i][2]));
  return res; }
