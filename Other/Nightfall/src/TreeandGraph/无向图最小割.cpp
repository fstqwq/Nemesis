...
int get(int&s,int&t){
	...
	fr(i,1,n){  k=mx=-1;
		fr(j,1,n)if(!g[j]&&!v[j]&&d[j]>mx)k=j,mx=d[j];
		if(k==-1)return an;
		s=t;t=k;an=mx;v[k]=1;
...
//n 为点数, w[i][j] 为 i 到 j 的流量, 返回无向图所有点对最小割之和 
	...
	fr(i,1,n-1){
		ans=min(ans,get(s,t));
		g[t]=1;if(!ans)break;
		fr(j,1,n)if(!g[j])w[s][j]=(w[j][s]+=w[j][t]);
	}return ans;}
// 无向图最小割树
void fz(int l,int r){// 左闭右闭, 分治建图 
	...
	ADD(S,T,fl);// 在最小割树中建边 
	L=l,R=r;fr(i,l,r) if(v[a[i]])q[L++]=a[i]; else q[R--]=a[i];
	fr(i,l,r)a[i]=q[i];fz(l,L-1);fz(R+1,r);}